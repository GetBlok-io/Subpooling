...
class PayoutGroupSuite extends AnyFunSuite{
  val balanceState = new BalanceState[SingleBalance]("test/payout_group_pool_test4")
...
//  payoutTx()
 ...
 // printMembers()
  ...
  def setup() = {
    ergoClient.execute {
      ctx =>
        val initStateBox = toInput(BalanceStateContract.buildBox(ctx, balanceState, dummyTokenId, dummyWallet.p2pk, PlasmaScripts.SINGLE))
