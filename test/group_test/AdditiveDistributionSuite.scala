...
import group_test.MockData.SingleDistributionData._
...
class AdditiveDistributionSuite extends AnyFunSuite {
    ...
    ergoClient.execute {
      ctx =>
        ...
        group = new DistributionGroup(singlePool, ctx, dummyWallet,
          commandContract, additiveHoldingContract, Some("COMET"),false)
  ...
  test("Make DistributionBuilder") {
    builder = new DistributionBuilder(initSingleAdditiveHoldingMap, Map(), Some(getInputBoxes), false)
  }
