...
import group_test.MockData.{dummyWallet, emissionsOperator, ergoClient, holdingContract}
import io.getblok.subpooling_core.boxes.{EmissionsBox, ExchangeEmissionsBox}
...
import io.getblok.subpooling_core.groups.stages.roots.{EmissionRoot, ExchangeEmissionsRoot}
...
class ExchangeHoldingSuite extends AnyFunSuite {
  var root: ExchangeEmissionsRoot = _
  ...
  var emissionsBox: ExchangeEmissionsBox = _
  ...
  test("Make Emissions Box"){
    emissionsBox = buildExchangeBox(totalEmissions, holdingContract)
  }

  test("Make Emissions Root") {
    ergoClient.execute{
      ctx =>
        root = new ExchangeEmissionsRoot(singlePool, ctx, dummyWallet, holdingContract, holdingValue, baseFeeMap, emissionsBox,
  ...
  test("Group placements valid"){
    group.poolPlacements.foreach{
      p =>
        ...
        assert(p.amount == TokenHoldingContract.getBoxValue(asMember.get.shareScore, subPool.nextTotalScore, root.totalTokensDistributed - (root.totalTokensDistributed / 100)),
