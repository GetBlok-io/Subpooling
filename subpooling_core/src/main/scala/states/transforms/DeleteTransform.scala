...
import contracts.plasma.DeleteBalanceContract
...
case class DeleteTransform[T <: StateBalance](override val ctx: BlockchainContext, override val wallet: NodeWallet,
  ...
  override def transform(state: State[T]): Try[TransformResult[T]] = {
    Try {
      ...
      val appliedCommand = DeleteBalanceContract.applyContext(commandState.box, state.balanceState,
        commandState.data.map(_.toStateMiner.toPartialStateMiner), state.isZeroed)
