...
case class PayoutTransform(override val ctx: BlockchainContext, override val wallet: NodeWallet, override val commandState: CommandState)
  ...
  override def transform(state: State): Try[TransformResult] = {
    Try{
      ...
      val appliedCommand = PayoutBalanceContract.applyContext(commandState.box, state.balanceState, commandState.data.map(_.toStateMiner))
      ...
      val nextStateBox = state.output(ctx, Some(state.box.getValue.toLong - totalRemoved))
      val payoutBoxes = PayoutBalanceContract.buildPaymentBoxes(ctx, appliedCommand._2)
      ...
      TransformResult(nextState, signedTx, commandState.data, CommandTypes.PAYOUT, Some(manifest))
    }
  }
