...
import contracts.emissions.{ExchangeContract, HybridExchangeContract}
 ...
 */
class HybridExchangeBox(box: InputBox, emissionsContract: HybridExchangeContract){
  val asInput: InputBox = box
  val contract: HybridExchangeContract = emissionsContract
  ...
  val percentChange: LongReg   = LongReg(this.getRegisters.head.getValue.asInstanceOf[Long])
  val poolFee: LongReg   = LongReg(this.getRegisters(1).getValue.asInstanceOf[Long])
  val proportion: LongReg = LongReg(this.getRegisters(2).getValue.asInstanceOf[Long])
  ...
  override def toString: String = {
    s"HybridExchangeBox[#${Helpers.trunc(this.emissionId.toString)}]" +
