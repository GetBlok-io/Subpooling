...
import org.ergoplatform.appkit.{Address, BlockchainContext, SignedTransaction}

class GenesisGroup(pool: Pool, ctx: BlockchainContext, wallet: NodeWallet, metadataVal: Long, feeValue: Int = 1000, feeOp: Option[Address] = None)
  extends TransactionGroup(pool, ctx, wallet) {
  ...
  override def executeGroup: TransactionGroup = {
    val result = stageManager.execute[MetadataInputBox](new GenesisStage(pool, ctx, wallet, metadataVal, feeValue, feeOp))
