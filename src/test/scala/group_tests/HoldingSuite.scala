...
import group_tests.groups.builders.{DistributionBuilder, HoldingBuilder}
...
import groups.{DistributionGroup, GroupManager, HoldingGroup}
...
class HoldingSuite extends AnyFunSuite {
  ...
  test("Make HoldingBuilder") {
    builder = new HoldingBuilder(holdingValue, holdingContract)
  }
