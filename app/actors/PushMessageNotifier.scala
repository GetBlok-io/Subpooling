...
import io.getblok.subpooling_core.persistence.models.PersistenceModels.{Block, PoolBlock, PoolInformation, PoolMember, PoolPlacement, PoolState}
      ...
      message match {
        case BlockMessage(poolBlock, poolInfo) =>
          if(params.enableBlockBot) {
            val effort =
              poolBlock.effort match {
              case Some(value) =>
                (value.toFloat * 100).toString + "%"
              case None =>
                "None"
            }
