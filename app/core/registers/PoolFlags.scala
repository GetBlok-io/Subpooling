...
class PoolFlags(val arr: Array[Long]) extends RegisterCollection[Long](arr){
  ...
  def withEpoch(epoch: Long)              = new PoolFlags(arr.updated(EPOCH_IDX, epoch))
  def withHeight(height: Long)            = new PoolFlags(arr.updated(HEIGHT_IDX, height))
  def withGenHeight(genHeight: Long)      = new PoolFlags(arr.updated(GEN_HEIGHT_IDX, genHeight))
  def withSubpool(subpool: Long)          = new PoolFlags(arr.updated(SUBPOOL_IDX, subpool))
  def withTag(tag: Long)                  = new PoolFlags(arr.updated(TAG_IDX, tag))
  def withAdditional(add: Array[Long])    = new PoolFlags(arr.slice(0, ADDITIONAL_START) ++ add)

  override def toString: String = s"PF[$getSubpool](${getEpoch}, ${getEpochHeight}, ${getGenesisHeight})" +
