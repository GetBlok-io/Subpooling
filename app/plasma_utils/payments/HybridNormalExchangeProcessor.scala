...
case class HybridNormalExchangeProcessor(settings: Seq[SMinerSettings], collector: ShareCollector, batch: BatchSelection, reward: Long, fee: Long,
                                         emReq: ActorRef)
                            extends PaymentProcessor {

  implicit val timeout: Timeout = Timeout(500 seconds)
  private val logger: Logger = LoggerFactory.getLogger("HybridNormalExchangeProcessor")
